<div class="modal fade" id="{{ 'eliminar'.$row }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <form class="jus" action="{{ route('EaGenArchiFinanController.destroy') }}" method="post">
    @csrf
        ...
        <div class="modal-body">
            <strong><p>{{'¿ Desea eliminar el registro de la carga corporativa ?'}}</strong></p>
            <ul>
                <li>{{ 'Del cliente: '.$data->cliente }}</li>
                <li>{{ 'Código de carga: '.  $data->cod_carga }}</li>
            </ul>
